...
import { FaPlus, FaMoneyBillWave, FaSearch, FaFilter, FaHistory, FaTrash, FaExclamationTriangle } from 'react-icons/fa'
...
export default function ReceivablesPage() {
  ...
  const fetchReceivables = async () => {
    ...
    try {
      ...
      const total = data.reduce((sum: number, r: any) => sum + r.remaining, 0)
      const lunas = data.filter((r: any) => r.status === 'LUNAS').reduce((sum: number, r: any) => sum + r.amount, 0)
      const belumLunas = data.filter((r: any) => r.status !== 'LUNAS').reduce((sum: number, r: any) => sum + r.remaining, 0)
      
      // Check jatuh tempo (overdue)
      const today = new Date()
      const jatuhTempo = data.filter((r: any) => {
        if (!r.dueDate || r.status === 'LUNAS') return false
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    if (paymentForm.amount > selectedReceivable.remaining) {
    ...
    try {
      const res = await fetch(`/api/receivables/${selectedReceivable.id}/payments`, {
        method: 'POST',
  ...
  const fetchPaymentHistory = async (receivableId: string) => {
    try {
  ...
  const handleDelete = async () => {
    ...
    const loadingToast = toast.loading('Menghapus piutang...')
    ...
    try {
      ...
      toast.error('Terjadi kesalahan saat menghapus piutang')
    }
  }
  ...
  const openHistoryModal = (receivable: any) => {
    setSelectedReceivable(receivable)
    fetchPaymentHistory(receivable.id)
  ...
  // Filter and search
  const filteredData = receivables.filter((item: any) => {
    const matchSearch = 
      ...
      <div className="container-responsive py-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2 ml-1">Kelola dan pantau piutang pelanggan</p>
          </div>
        </div>

        {/* Statistics Cards */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-gradient-to-br from-yellow-400 to-orange-500 text-white p-6 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
          ...
          <div className="bg-gradient-to-br from-blue-500 to-indigo-600 text-white p-6 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
            ...
            <h3 className="text-sm font-semibold opacity-90 uppercase tracking-wide">Total Piutang</h3>
            <p className="text-3xl font-bold mt-2">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-lg mb-8 border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <Input
                type="text"
                placeholder="Cari nama customer atau keterangan..."
                value={searchTerm}
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-2.5 border-2 border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all font-medium"
              >
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
          {loading ? (
            <div className="flex flex-col justify-center items-center h-64">
              ...
              <p className="mt-4 text-gray-600 font-medium">Memuat data piutang...</p>
            </div>
            ...
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 text-lg font-medium">Tidak ada data piutang</p>
              <p className="text-gray-400 text-sm mt-2">Data akan muncul setelah transaksi penjualan dengan sistem piutang</p>
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentData.map((item: any) => (
                    <tr key={item.id} className={`transition-all duration-200 hover:bg-blue-50 hover:shadow-md ${isOverdue(item.dueDate, item.status) ? 'bg-red-50 border-l-4 border-red-500' : 'hover:border-l-4 hover:border-blue-500'}`}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        {isOverdue(item.dueDate, item.status) && (
                          <FaExclamationTriangle className="text-red-500 flex-shrink-0" />
                        )}
                        <span className="font-semibold text-gray-900">{item.customer || 'N/A'}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
      ...
      >
        <form onSubmit={handlePayment}>
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">Sisa Piutang</label>
              <Input
                type="text"
                value={`Rp ${(selectedReceivable?.remaining || 0).toLocaleString('id-ID')}`}
                disabled
                className="bg-gray-100"
              />
            </div>
