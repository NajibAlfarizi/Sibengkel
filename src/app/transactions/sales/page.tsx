...
export default function SalesPage() {
  ...
  const fetchTransactions = async () => {
    setLoading(true)
    try {
      let url = '/api/transactions?type=PENJUALAN'
      if (statusFilter !== 'ALL') {
  ...
  const columns = [
    ...
    },
    {
      key: 'customer',
      label: 'Customer',
      render: (_: any, row: any) => row.customer?.name || 'Umum',
    },
    {
      key: 'total',
      label: 'Total',
    ...
    },
    {
      ...
      render: (value: string) => (
        <span className={`px-2 py-1 rounded-full text-xs ${
          value === 'CASH' ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800'
        }`}>
          {value}
        </span>
      ),
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-gray-800">Transaksi Penjualan</h1>
